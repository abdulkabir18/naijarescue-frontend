...
//                     window.location.href = `responder-incident-details.html?id=${notification.targetId}`;
//                 }
//                 else if (userRole === "AgencyAdmin"){
//                     window.location.href = `agency-incident-details.html?id=${notification.targetId}`;
//                 }
//                 else {
...
const fallbackSounds = [
    ...
    "https://cdn.pixabay.com/download/audio/2022/03/10/audio_c73ab19d1a.mp3", 
    "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGnOL0t2krCCqB0PLaizkGHm7A7+OZURE" 
];
...
class NotificationManager {
    ...
    setupNotificationDropdown() {
        ...
        if (markAllRead) {
            markAllRead.addEventListener("click", async () => {
                const token = sessionStorage.getItem("authToken");
                await this.markAllNotificationsAsRead(token);
            });
        }
    }
    ...
    playNotificationSound() {
                ...
                playPromise
                    ...
                    .catch(error => {
                        ...
                        if (index < fallbackSounds.length) {
                            ...
                            console.warn("All notification sounds failed. User may need to interact with page first.");
                        }
                    });
            }
        };
        ...
        try {
            playSound(notificationSound.src, 0);
        } catch (error) {
    ...
    getNotificationPageUrl(userRole) {
        const urlMap = {
            "Victim": "notifications.html",
            "Responder": "responder-notifications.html",
            "AgencyAdmin": "agency-notifications.html",
            "SuperAdmin": "admin-notifications.html"
        };
        return urlMap[userRole] || "notifications.html";
    }
    ...
    createNotificationElement(notification) {
        ...
        div.addEventListener("click", async () => {
            ...
            if (notification.targetType?.toLowerCase() === "incident" && notification.targetId) {
                const userRole = getRole(token);
                
                const incidentUrls = {
                    "SuperAdmin": `incident-details.html?id=${notification.targetId}`,
                    "Victim": `report-details.html?id=${notification.targetId}`,
                    "Responder": `responder-incident-details.html?id=${notification.targetId}`,
                    "AgencyAdmin": `agency-incident-details.html?id=${notification.targetId}`
                };
                
                const url = incidentUrls[userRole];
                if (url) {
    ...
    async markAllNotificationsAsRead(token) {
        try {
            ...
            const response = await fetch(`${AppConfig.API_BASE_URL}/api/v1/Notification/user/${userId}/mark-all-read`, {
                method: "PATCH",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Accept": "application/json"
