document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!responderId) {
        detailsContainer.innerHTML = `<div class="error-state"><p>No responder ID provided.</p><a href="responders.html" class="btn-primary">Back to Responders</a></div>`;
        return;
    }
    ...
    async function loadResponderDetails() {
        try {
            ...
            console.error("Error loading responder details:", error);
            detailsContainer.innerHTML = `<div class="error-state"><p>${error.message}</p><a href="responders.html" class="btn-primary">Back</a></div>`;
        }
    }
    ...
    function loadGoogleMapsApi() {
        return new Promise((resolve, reject) => {
            ...
            script.src = `https://maps.googleapis.com/maps/api/js?key=${AppConfig.GOOGLE_MAPS_API_KEY}&libraries=places&callback=initMapDetails`;
            script.async = true;
    ...
    function setupMap(coords) {
        const defaultLocation = { lat: 6.5244, lng: 3.3792 }; // Default to Lagos
        const initialLocation = (coords && coords.latitude) ? { lat: coords.latitude, lng: coords.longitude } : defaultLocation;

        map = new google.maps.Map(locationMapEl, {
            center: initialLocation,
            zoom: (coords && coords.latitude) ? 15 : 10,
    ...
    statusForm.addEventListener('submit', async (e) => {
        ...
        const newStatusString = document.getElementById('responderStatus').value;
        const newStatusInt = responderStatusEnum[newStatusString];
