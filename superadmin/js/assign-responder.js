document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function loadAvailableResponders(incident) {
        try {
            // Fetch all responders and filter by availability and supported incident type
            ...
            if (result.succeeded && result.data) {
                // Assuming ResponderDto has `status` and `agency.supportedIncidents`
    ...
    function filterAndDisplayResponders() {
        ...
        const filteredResponders = allResponders.filter(responder => {
            ...
            const agencyName = `${responder.agencyName}`.toLowerCase();
            return fullName.includes(searchTerm) || agencyName.includes(searchTerm);
        });
    ...
    function displayResponders(responders) {
        ...
        respondersListContainer.innerHTML = responders.map(responder => {
            const avatarSrc = `${AppConfig.API_BASE_URL}/${responder.profilePictureUrl || generateInitialsAvatar(responder.userFullName)}`;
            return `
                <div class="responder-card">
                    <div class="responder-info">
                        ...
                        <div>
                            ...
                            <div class="responder-agency">${escapeHtml(responder.agencyName)}</div>
                        </div>
                    </div>
                    <div class="responder-status">
                        <span class="status-badge-small available">${responder.status}</span>
                    </div>
    ...
    async function handleAssignResponder(event) {
        ...
        button.innerHTML = '<div class="spinner-small-white"></div> Assigning...';
        ...
        if (!incidentId) {
            document.querySelector('.admin-main').innerHTML = `
                <div class="error-state">
                    ...
                    <a href="incidents.html" class="btn-primary">Go to Incidents</a>
                </div>
            `;
            return;
        }

        backLink.href = `incident-details.html?id=${incidentId}`;
        loadIncidentSummary();
    }
